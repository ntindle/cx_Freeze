...
if [ -z "$1" ]; then
    echo "Usage:"
    echo "$0 sample [--pipenv|--venv|--system] [--deps=x] [--debug] [--verbose]"
    echo "Where:"
...
TEST_VERBOSE=""
while ! [ -z "$2" ]; do
    if [ "$2" == "--pipenv" ] || [ "$2" == "--venv" ] || [ "$2" == "--system" ]; then
        TEST_VENV="$2"
    elif [ "$2" == "--deps=b" ]; then
...
if [ -f setup.py ]; then
    ...
    PY_SCRIPT=$($PYTHON -c "import sysconfig; print(sysconfig.get_path('scripts'))")
...
for i in 0 1 2; do
    ...
    while IFS= read -r TEST_RUN; do
        ...
        if [[ $TEST_NAME == b64:* ]]; then
            ...
            TEST_NAME=$($PYTHON -c "from base64 import b64decode; print(b64decode(\"$TEST_B64\").decode())")
        fi
