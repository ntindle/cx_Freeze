...
PIP_COMMAND = (
    [which("uv"), "pip"] if which("uv") else [sys.executable, "-m", "pip"]
)
...
def is_supported_python(python_version: str) -> bool:
    __import__(f"{packaging.__name__}.specifiers")
    Specifier = packaging.specifiers.Specifier
    return PYTHON_VERSION in Specifier(python_version)
...
def install_requirements(
    ...
    __import__(f"{packaging.__name__}.requirements")
